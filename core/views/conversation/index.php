...
<?php if ($data["searchString"]): ?><a href='<?php echo URL(conversationURL($conversation["conversationId"], $conversation["title"])); ?>' class='control-reset'>x</a><?php endif; ?>
</fieldset>
</form>
...
foreach ($scrubber as $year => $months) {
	...
	if (!empty($months)) {
		...
		foreach ($months as $month) {
			...
			$oldestMonth = date("n", $conversation["startTime"]);#
			$name = strftime("%B", mktime(0, 0, 0, $month, 1));
			if($name =='December') {$name='Detsember';}
			if($name =='November') {$name='November';}
			if($name =='October') {$name='Oktoober';}
			if($name =='September') {$name='September';}
			if($name =='August') {$name='August';}
			if($name =='July') {$name='Juuli';}
			if($name =='June') {$name='Juuni';}
			if($name =='May') {$name='Mai';}
			if($name =='April') {$name='Aprill';}
			if($name =='March') {$name='Märts';}
			if($name =='February') {$name='Veebruar';}
			if($name =='January') {$name='Jaanuar';}
...
<li class='scrubberMore scrubberPrevious'><a href='<?php echo URL(makeURL("p".(ceil($data["startFrom"] / C("esoTalk.conversation.postsPerPage") + 1) - 1), $data["searchString"])); ?>'>&lsaquo; <?php echo T("Kuva vanemad"); ?></a></li>
<?php endif; ?>
...
<li class='scrubberMore scrubberNext'><a href='<?php echo URL(makeURL("p".(floor($data["startFrom"] / C("esoTalk.conversation.postsPerPage") + 1) + 1), $data["searchString"])); ?>'><?php echo T("Kuva uuemad"); ?> &rsaquo;</a></li>
<?php endif; ?>
