...
public function index($conversationId = false, $year = false, $month = false)
{
	...
	if ($this->responseType === RESPONSE_TYPE_DEFAULT) {
		...
		if ($conversation["canModerate"]) {
			...
			$controls->add("ootel", "<a href='".URL("conversation/ootele/".$conversation["conversationId"]."/?token=".ET::$session->token."&return=".urlencode($this->selfURL))."' id='control-glass'><i class='icon-glass'></i> <span>".T($conversation["ootel"] ? "Eemalda ootel" : "Pane ootele")."</span></a>");
			$controls->add("probleem", "<a href='".URL("conversation/probleem/".$conversation["conversationId"]."/?token=".ET::$session->token."&return=".urlencode($this->selfURL))."' id='control-warning'><i class='icon-warning-sign'></i> <span>".T($conversation["probleem"] ? "Eemalda probleemsus" : "Probleemne")."</span></a>");
...
protected function formatPostForTemplate($post, $conversation)
{
	...
	elseif (time() - $post["time"] < 180 * 24 * 60 * 60)
		...
		$date = date("M", $post["time"]);
		if($date == 'Dec'){$date='detsember '.	date("j",$post["time"]);}
		if($date == 'Jan'){$date='jaanuar '.	date("j",$post["time"]);}
		if($date == 'Feb'){$date='veebruar '.	date("j",$post["time"]);}
		if($date == 'Mar'){$date='märts '.	date("j",$post["time"]);}
		if($date == 'Apr'){$date='aprill '.	date("j",$post["time"]);}
		if($date == 'May'){$date='mai '.	date("j",$post["time"]);}
		if($date == 'Jun'){$date='juuni '.	date("j",$post["time"]);}
		if($date == 'Jul'){$date='juuli '.	date("j",$post["time"]);}
		if($date == 'Aug'){$date='august '.	date("j",$post["time"]);}
		if($date == 'Sep'){$date='september '.	date("j",$post["time"]);}
		if($date == 'Okt'){$date='oktoober '.	date("j",$post["time"]);}
		if($date == 'Nov'){$date='november '.	date("j",$post["time"]);}
	...
	// Otherwise, show the month, day, and year.
		...
		$date = date("M", $post["time"]);
		if($date == 'Dec'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Jan'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Feb'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Mar'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Apr'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'May'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Jun'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Jul'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Aug'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Sep'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Okt'){$date=date("Y-m-d",$post["time"]);}
		if($date == 'Nov'){$date=date("Y-m-d",$post["time"]);}
...
protected function getPostForEditing($postId)
{
	...
	$aruteluid = ET::SQL()
		...
		//Kontrollitakse kas arutelu on juba ootel.
		$model = ET::conversationModel();
		if($aruteluid==0){
	...
	public function probleem($conversationId)
	{
	$aruteluid = ET::SQL()
		->select("probleem")
		->from("conversation")
		...
		$model = ET::conversationModel();
		if($aruteluid==0){
		$model->updateById($conversationId, array("probleem" => 1));
		}else{
		$model->updateById($conversationId, array("probleem" => 0));
		}

		// menüü lipikud sisse-välja
		$pooleli = !$conversation["probleem"];
		ET::conversationModel()->setiprobleem($conversation, ET::$session->userId, $pooleli);
